...
export function LegacyDnaPanel({ runId }: PanelProps) {
  return (
    <AsyncPanel
      title="Legacy DNA"
      load={() => api.getLegacyDna(runId)}
      deps={[runId]}
      isEmpty={(r) => r.length === 0}
      emptyText="No Legacy DNA rows for this run."
    >
      {(rows) => (
        <>
          <TableScroll>
          ...
          <div className="meta" style={{ marginTop: 6 }}>
            * coverage is a proxy (presence of a test file), not measured coverage — a
            measured run (FULL mode) replaces it with a real fraction.
          </div>
        </>
      )}
    </AsyncPanel>
